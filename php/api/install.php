<?php
/**
 * One-time installer — creates the first admin user from config.php, verifies
 * the DB connection and tables, then SELF-DELETES. Hit it once in a browser:
 *   https://app.cosmos.in/api/install.php
 */
require_once __DIR__ . '/lib/helpers.php';

header('Content-Type: text/plain; charset=utf-8');
$cfg = config();

try {
    $pdo = db();
    echo "✓ Connected to MySQL ({$cfg['db']['name']})\n";

    // Verify the expected tables exist.
    $need = ['users','uploads','cards','ocr_results','companies','leads','contacts','duplicates','exports','audit_logs'];
    $have = $pdo->query('SHOW TABLES')->fetchAll(PDO::FETCH_COLUMN);
    $missing = array_diff($need, $have);
    if ($missing) { echo "✗ Missing tables: " . implode(', ', $missing) . "\n"; exit; }
    echo "✓ All " . count($need) . " tables present\n";

    // Seed (or reset) the admin user.
    $email = strtolower(trim($cfg['admin_email']));
    $hash  = password_hash($cfg['admin_password'], PASSWORD_BCRYPT);
    $existing = q1('SELECT id FROM users WHERE email = ?', [$email]);
    if ($existing) {
        exec_sql('UPDATE users SET passwordHash = ?, role = ?, isActive = 1, updatedAt = ? WHERE id = ?',
            [$hash, 'ADMIN', now3(), $existing['id']]);
        echo "✓ Admin user updated: $email\n";
    } else {
        exec_sql('INSERT INTO users (id, email, passwordHash, name, role, isActive, createdAt, updatedAt)
                  VALUES (?,?,?,?,?,1,?,?)',
            [uuid_v4(), $email, $hash, 'Cosmos Admin', 'ADMIN', now3(), now3()]);
        echo "✓ Admin user created: $email\n";
    }

    echo "✓ Groq key " . (config()['groq_api_key'] ? 'is set' : 'NOT set (add it in config.php before scanning)') . "\n";
    echo "\nSetup complete. Log in at https://app.cosmos.in\n";
    echo "Deleting this installer for security...\n";
    @unlink(__FILE__);
    echo file_exists(__FILE__) ? "⚠ Could not self-delete — please remove api/install.php manually.\n"
                               : "✓ Installer removed.\n";
} catch (Throwable $e) {
    echo "✗ ERROR: " . $e->getMessage() . "\n";
}
